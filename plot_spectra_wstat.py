...
for i in dirs:
    ...
    #        group_counts(10)
            ...
            ufit = pyfits.open(i+'/srcflux_output/sers_{0:4d}_wstat_umod.pi'.format(j).replace(' ','0'))
            afit = pyfits.open(i+'/srcflux_output/sers_{0:4d}_wstat_amod.pi'.format(j).replace(' ','0'))
            data = get_data()
    ...
    #            dat.append(sum(data[1].data['MATRIX'][m]))
            ...
            fig.savefig('spectrum_plots/wstat/e_{1:5d}_src_{0:4d}_fit.png'.format(j,int(i)).replace(' ','0'),bbox_pad=.1,bbox_inches='tight')
            plt.clf()
            plt.close()
        except:
            print '' 
